...
for response in tweepy.Paginator(client.search_recent_tweets, query=query, tweet_fields=['context_annotations', 'created_at'], user_fields=['profile_image_url', 'username'], expansions='author_id', max_results=100).flatten(limit=1000):
    tweets= response.data
    users = response.includes
