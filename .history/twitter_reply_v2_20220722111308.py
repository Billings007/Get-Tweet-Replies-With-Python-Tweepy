...
tweets=[]
temp = client.search_recent_tweets(query=query, tweet_fields=['context_annotations', 'created_at'],user_fields=['profile_image_url', 'username'], expansions='author_id')
users = {u["id"]: u for u in tweets.includes['users']}

with open('replies_clean.csv', 'w') as f:
    csv_writer = csv.DictWriter(f, fieldnames=('user', 'text'))
    csv_writer.writeheader()
    for tweet in temp.data:
        row = {'user': tweet.username, 'text': tweet.text.replace('\n', ' ')}
